...
class Section(BaseModel):
    """Represents a meaningful section of a document."""
...
def get_structured_document(document_with_line_numbers: str, max_retries: int = 3) -> StructuredDocument:
    ...
    system_prompt = """You are a world class legal document analyzer working on organizing contract sections.
Read the document below and extract a StructuredDocument object from it where each section represents a distinct legal concept or clause that would be reviewed separately by a lawyer.
...
Focus on creating sections around legal concepts such as:
...
Make sure sections are meaningful and self-contained."""
...
def get_sections_text(structured_doc: StructuredDocument, line2text: dict[int, str]) -> List[dict]:
    """
    Extract the actual text content for each section.
    ...
    Returns:
        List of dictionaries containing section info and text
    """
    sections = []
